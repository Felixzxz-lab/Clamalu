...
export default requireAdmin(async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await db.from('usuarios').select('id,nome,email,role,ativo,paginas,criado_em').order('criado_em', { ascending: false })
    if (error) return res.status(500).json({ error: error.message })
    return res.status(200).json(data)
  }

  if (req.method === 'POST') {
    const { nome, email, senha, role, paginas } = req.body
  ...
  if (req.method === 'PUT') {
    const { id, nome, email, senha, role, paginas, ativo } = req.body
