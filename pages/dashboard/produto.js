    ...
    <div style={{ minHeight:'100vh',background:'#f4f6fb',fontFamily:"'Segoe UI',system-ui,sans-serif",fontSize:13 }}>
      ...
      <div style={st.filtros}>
        <div style={{ display:'flex',alignItems:'center',gap:7 }}><span style={st.label}>Ano</span>
          <select style={{ ...st.select,borderColor:'#a3b4f5' }} value={fAno} onChange={e=>setFAno(e.target.value)}><option value="">Todos</option><option>2024</option><option>2025</option><option>2026</option></select></div>
        <div style={{ display:'flex',alignItems:'center',gap:7 }}><span style={st.label}>Mês</span>
          <select style={{ ...st.select,borderColor:'#f5a3a3' }} value={fMes} onChange={e=>setFMes(e.target.value)}><option value="">Todos</option>{['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'].map((m,i)=><option key={i} value={i+1}>{m}</option>)}</select></div>
        <div style={{ display:'flex',alignItems:'center',gap:7 }}><span style={st.label}>Vendedor</span>
          <select style={{ ...st.select,borderColor:'#a3dbb4' }} value={fVend} onChange={e=>setFVend(e.target.value)}><option value="">Todos</option><option>THIAGO</option><option>WENDEL</option><option>CLEBER</option><option>CLAMALU</option></select></div>
