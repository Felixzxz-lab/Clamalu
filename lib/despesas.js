...
function acharCabecalho(matriz) {
  for (let i = 0; i < Math.min(matriz.length, 8); i++) {
    ...
    // A 1a coluna e o fornecedor. O rotulo dela varia entre as planilhas:
    // "DESPESA" (matriz do ano) ou "FAVORECIDO" (resumo do mes, onde a coluna
    // "DESPESA" passa a ser a 2a, com o tipo).
    if (c0.startsWith('despesa') || c0.startsWith('favorecido') || c0.startsWith('fornecedor')) {
      const colMes = {}
